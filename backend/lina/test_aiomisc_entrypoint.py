...
os.environ["SILICON_FLOW_API_KEY"] = "k-test"
...
def test_entrypoint_start_stop():
    ...
    async def smoke():
        services = [
            lina_service.VoicePoolService(default_provider="siliconflow"),
        ...
        async with aiomisc.entrypoint(*services):
            ...
            assert primary is not None and primary.name == "siliconflow", pool.names
...
def test_voice_pool_service_env_driven():
    ...
    os.environ["SILICON_FLOW_API_KEY"] = "k-test"
    os.environ["HUGGING_FACE_ACCESS_TOKEN"] = "k-test"
    os.environ.pop("AI_PROVIDERS", None)

    async def run():
        async with aiomisc.entrypoint(
            lina_service.VoicePoolService(default_provider="siliconflow", max_concurrent=7)
        ):
            pool = lina_service._context_get("voice_pool")
            assert pool is not None
